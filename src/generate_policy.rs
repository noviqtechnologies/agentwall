...
pub fn generate_from_events(events: &[Event]) -> String {
    ...
    for event in events {
        ...
        if let Ok(params_val) = serde_json::from_str::<serde_json::Value>(&event.parameters) {
            let mut flat: Vec<(String, serde_json::Value)> = Vec::new();
            flatten_json(&params_val, "", 0, 5, &mut flat);

            for (key, val) in flat {
                let stats = analysis.params.entry(key).or_default();
    ...
    let global_first = events.iter().map(|e| e.timestamp.as_str()).min().unwrap_or("");
    let global_last = events.iter().map(|e| e.timestamp.as_str()).max().unwrap_or("");

    // Trim to date portion (first 10 chars of ISO 8601 timestamp)
    let window_start = if global_first.len() >= 10 { &global_first[..10] } else { global_first };
    let window_end = if global_last.len() >= 10 { &global_last[..10] } else { global_last };
    ...
    let mut out = format!(
        "# Auto-generated by AgentWall from {} observed tool calls\n\
         ...
         default_action: deny\n\n\
         tools:\n",
        total_events, window_start, window_end, tool_count
    );
    ...
    for tool_name in &sorted_tools {
        ...
        out.push_str(&format!(
            ...
            tool_name, effective_risk_tier, confidence, call_count
        ));
...
mod tests {
    ...
    fn make_event(tool: &str, params: &str) -> Event {
        Event {
