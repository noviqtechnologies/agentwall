...
### 🛡️ Safe Mode — Out-of-the-Box Protection (FR-303a)
* **Enabled by default.** Safe Mode applies 15 high-signal rules that block dangerous tool calls without any policy configuration. It is tool-aware: each rule targets only the relevant parameter type (file path, command, or URL), minimizing false positives.
...
* **Dangerous Commands (4 rules):**
  - Blocks pipe-to-shell patterns (`curl … | bash`, `wget … | sh/python/perl/ruby`).
...
Safe Mode runs before the policy engine — it protects agents even in shadow mode (`agentwall dev`) where no policy file is loaded. To inspect the full rule set, see `src/policy/safe_mode.rs`.
...
```
The chart deploys the operator, gateway, RBAC, CRD, and optional NetworkPolicy enforcement. To also deploy the SaaS Dashboard components, add `--set dashboardApi.enabled=true --set dashboardDb.enabled=true --set dashboardFrontend.enabled=true` — see [SaaS Dashboard (FR-23)](#saas-dashboard--fleet-overview-identity-governance--policy-insights-fr-23) for the full example. See `chart/README.md` and `chart/values.yaml` for the full reference.
...
- [x] **AC-5.9:** Integration test suite (263 tests)
...
## SaaS Dashboard — Fleet Overview, Identity Governance & Policy Insights (FR-23)
...
| Safe Mode (15 rules, FR-303a) | ✅ Built | Phase 1 |
